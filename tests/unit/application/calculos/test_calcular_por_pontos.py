...
Usa fake in-memory para :class:`LeitorNetCDF` — não toca banco nem arquivos
reais. O caso de uso é puro: apenas calcula e devolve
:class:`ResultadoCalculo`. A persistência vive em
:class:`ProcessarPontosLote` (testes em
``tests/unit/application/calculos/test_processar_pontos_lote.py``).
"""
...
async def test_executar_dois_pontos_tres_anos_ignora_ano_ausente() -> None:
    ...
    caso = CalcularIndicesPorPontos(leitor)  # type: ignore[arg-type]
