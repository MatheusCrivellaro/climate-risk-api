"""Testes de :meth:`CalcularIndicesEstresseHidrico.executar_de_pasta` (Slice 17)."""
...
def _params(
    pasta_pr: Path, pasta_tas: Path, pasta_evap: Path, *, cenario: str = "rcp45"
) -> ParametrosCalculoEstresseHidricoPasta:
...
async def test_executar_de_pasta_cria_execucao_e_enfileira_job_pasta(
    ...
    assert primeira.arquivo_origem == str(pasta_pr)
...
@pytest.mark.asyncio
async def test_pasta_inexistente_falha_antes_de_persistir(tmp_path: Path) -> None:
    pasta_pr = tmp_path / "pr"
    pasta_tas = tmp_path / "tas"
    pasta_pr.mkdir()
    pasta_tas.mkdir()
    ...
    with pytest.raises(ErroArquivoNCNaoEncontrado):
        await caso.executar_de_pasta(_params(pasta_pr, pasta_tas, pasta_evap_inexistente))
...
@pytest.mark.asyncio
async def test_pasta_que_eh_arquivo_falha(tmp_path: Path) -> None:
    """Se o caminho existe mas é arquivo (não diretório), também falha."""
    pasta_pr = tmp_path / "pr"
    pasta_tas = tmp_path / "tas"
    pasta_pr.mkdir()
    pasta_tas.mkdir()
    pasta_evap_arquivo = tmp_path / "evap.txt"
    pasta_evap_arquivo.write_text("não é pasta")
    ...
    with pytest.raises(ErroArquivoNCNaoEncontrado):
        await caso.executar_de_pasta(_params(pasta_pr, pasta_tas, pasta_evap_arquivo))
