"""Testes unitários de :class:`CalcularIndicesEstresseHidrico` (Slice 15)."""
...
def _params(
    ...
    *,
    ...
    return ParametrosCalculoEstresseHidrico(
        arquivo_pr=arquivo_pr,
        arquivo_tas=arquivo_tas,
        arquivo_evap=arquivo_evap,
        cenario=cenario,
...
async def test_cria_execucao_pending_e_enfileira_job(tmp_path: Path) -> None:
    ...
    resultado = await caso.executar(_params(pr, tas, evap))
    ...
    assert job.payload["arquivo_pr"] == str(pr)
    assert job.payload["arquivo_tas"] == str(tas)
    assert job.payload["arquivo_evap"] == str(evap)
...
@pytest.mark.asyncio
async def test_arquivo_pr_inexistente_falha_antes_de_persistir(tmp_path: Path) -> None:
    pr = tmp_path / "pr_ausente.nc"
    tas = tmp_path / "tas.nc"
    evap = tmp_path / "evap.nc"
    tas.write_bytes(b"")
    evap.write_bytes(b"")
    ...
    with pytest.raises(ErroArquivoNCNaoEncontrado):
        await caso.executar(_params(pr, tas, evap))
...
@pytest.mark.asyncio
async def test_arquivo_evap_inexistente_falha(tmp_path: Path) -> None:
    pr = tmp_path / "pr.nc"
    tas = tmp_path / "tas.nc"
    evap = tmp_path / "evap_ausente.nc"
    pr.write_bytes(b"")
    tas.write_bytes(b"")
    repo = _RepoExecucoesFake(salvos=[])
    fila = _FilaJobsFake(enfileirados=[])
